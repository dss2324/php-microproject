...
{   error_log("not working");
    $sno = $_GET['delete'];
    $delete=true;
    $sql="DELETE FROM `$task_table_name` WHERE `task_id`=$sno";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['snoEdit'])) {
        ...
        $sql = "INSERT INTO `$task_table_name` (`task_name`,`task_description`,`user_email`) VALUES ('$task_name','$task_description','$user_email')";
        $result = mysqli_query($connection, $sql);
...
<body>
  ...
  Launch demo modal
    ...
    if ($insert) {
        ...
        <strong>Task Saved</strong> refer it whenever you need.
    ...
    if ($update) {
        echo "<div class='alert alert-success alert-dismissible fade show' role='alert'>
        <strong>Task updated</strong> refer it whenever you need.
        <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
      </div>";
    }
    ?>
    ...
    <div class="container my-4">
        <form action="/task_manager2.php" method="post">
            <h2>Add a Task</h2>
            <div class="form-group">
                <label for="title">Task Name</label>
        ...
        <table class="table" id="myTable">
            ...
            <tbody>
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    <td><button class='edit btn btn-sm btn-primary' id=" . $row['task_id'] . ">Edit</button> <button class='delete btn btn-sm btn-primary' id=d".$row['task_id'].">Delete</button></td>
                </tr>";
                }
    ...
    <script>
        ...
        Array.from(deletes).forEach((element) => {
            element.addEventListener("click", (e) => {
                ...
                {
                    console.log("yes");
                    window.location=`/task_manager2.php?delete=${sno}`;
                }
                else{
                    console.log("no");
                }
